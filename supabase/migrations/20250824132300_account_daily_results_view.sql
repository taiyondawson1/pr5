...
intraday as (
  select
    ...
    p.day,
    p.cum,
    max(p.cum) over (partition by p.account_number, p.day order by p.exit_time rows between unbounded preceding and current row) as peak
  from path p
),
dd as (
  select
    i.account_number,
    i.day,
    max(case when i.peak > 0 then (i.peak - i.cum) / i.peak * 100 else 0 end) as drawdown_pct
  from intraday i
